  ...
  return (
    <div
      className={`w-72 rounded-2xl bg-white overflow-hidden select-none transition-all duration-150 ${
        selected
          ? "border-2 border-purple-400 shadow-xl shadow-purple-100/60"
          : "border-2 border-purple-100 shadow-md hover:shadow-lg"
      }`}
    >
      <NodeToolbarActions id={id} selected={selected} accentColor="bg-purple-500" />
      <Handle type="target" position={Position.Top} className="!bg-purple-400 !w-3 !h-3 !border-2 !border-white" />
      ...
      <div className="px-5 py-3">
        {hasFilter ? (
          <div className="rounded-lg bg-purple-50 px-3 py-2 font-mono text-xs text-purple-800">
            <span className="font-semibold">{column}</span>
            {" "}
            <span className="text-purple-400">{OP_LABELS[operator ?? ""] || operator}</span>
            {" "}
          ...
          <p className="text-sm leading-snug text-gray-400">
            Filter leads by matching column values before processing.
          </p>
        )}
      </div>
