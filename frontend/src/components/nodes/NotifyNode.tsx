...
export default function NotifyNode({
  ...
  id: string;
  data: { label?: string; config?: { channel?: string; message?: string } };
  selected: boolean;
}>) {
    ...
    <div className="relative select-none animate-node-enter" style={{ width: 260 }}>
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-slate-400">Internal Alert · {channelLabel}</p>
        </div>
      </div>

      <div className="px-1 pt-3 space-y-2">
        {message ? (
          <p className="truncate text-[12px] leading-relaxed text-slate-600 bg-emerald-50 rounded-lg px-2.5 py-1.5 border border-emerald-100">
            {message.length > 60 ? message.slice(0, 60) + "…" : message}
          </p>
        ) : (
          <p className="text-[12px] leading-relaxed text-slate-500">
            Send an internal alert to yourself when a lead reaches this step.
          </p>
        )}
        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold ${
          message ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"
        }`}>
          {message
            ? <><CheckCircle2 className="h-3 w-3" />via {channelLabel}</>
