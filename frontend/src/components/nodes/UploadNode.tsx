  ...
  return (
    <div
      className={`w-72 rounded-2xl bg-white overflow-hidden select-none transition-all duration-150 ${
        selected
          ? "border-2 border-blue-400 shadow-xl shadow-blue-100/60"
          : "border-2 border-blue-100 shadow-md hover:shadow-lg"
      }`}
    >
      <NodeToolbarActions id={id} selected={selected} accentColor="bg-blue-500" />
          ...
          <div className="space-y-1.5">
            {cols.slice(0, 3).map(([field, col]) => (
              <div key={field} className="flex items-center gap-2">
                ...
                <span className="truncate text-xs text-gray-400">← "{col}"</span>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-sm leading-snug text-gray-400">
            Import leads from CSV or Excel files to start your workflow.
          </p>
        )}
      </div>
      ...
      <div className="flex items-center px-5 pb-4 pt-1">
        <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ${
          cols.length > 0 ? "bg-blue-50 text-blue-600" : "bg-gray-100 text-gray-500"
        }`}>
          {cols.length > 0
            ? <><CheckCircle2 className="h-3 w-3" />&nbsp;{cols.length} columns</>
