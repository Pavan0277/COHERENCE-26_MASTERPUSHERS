...
export default function NodeConfigPanel({
        ...
        switch (node.type) {
            ...
            case "send":
                return (
                    <SendPanel
                        config={
                            config as { platform?: string; message?: string }
                        }
                        onChange={(c) =>
                            onUpdateConfig(
                                node.id,
            ...
            case "wait_until":
                return (
                    <WaitUntilPanel
                        config={
                            config as { datetime?: string; timezone?: string }
                        }
                        onChange={(c) =>
                            onUpdateConfig(
                                node.id,
    ...
    return (
        <div className="flex h-full flex-col">
            {/* Header */}
            <div
                className={`flex items-center justify-between bg-gradient-to-r ${TYPE_COLORS[node.type] || "from-gray-500 to-gray-600"} px-4 py-3.5`}
            >
                <div>
